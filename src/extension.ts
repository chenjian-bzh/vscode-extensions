...
function createTerminals(
  tasks: Array<{ name: string; text: string }>
): vscode.Terminal[] {
  return tasks.map(t => {
...
function getWorkSpace(): vscode.WorkspaceFolder {
  ...
  const vconsoleFolder = folders?.find((folder, idx) => {
    try {
      const pkg = fs
        .readFileSync(resolve(folder.uri.path, 'package.json'))
        .toString();
      if (pkg) {
...
function initDebugConfig(wf: vscode.WorkspaceFolder): string {
  ...
  if (fs.existsSync(configPath)) {
    missionName =
      missionName ||
      JSON.parse(fs.readFileSync(configPath).toString()).configurations.name;
  } else {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.launch.dev', () => {
      vscode.window.showQuickPick(products).then(val => {
        ...
        const webCommand =
          `yarn dev` + (val === 'all' ? '' : ` --web-module ${val}`);

        createTerminals([
          { name: 'web', text: `cd ${rootpath}/packages/web && ${webCommand}` },
          {
            name: 'platform',
            text: `cd ${rootpath}/packages/platform && yarn dev`
          }
        ]);
      });
    }),
    ...
    vscode.commands.registerCommand('extension.launch.debug', () => {
      vscode.window.showQuickPick(products).then(val => {
        ...
        createTerminals([
          {
            name: 'web',
            text: `cd ${wf.uri.path}/packages/web && yarn dev --web-module ${val}`
          }
        ]);
    ...
    vscode.commands.registerCommand(
      ...
      () => {
        try {
          ...
          initDebugConfig(wp);
          vscode.window.showInformationMessage(
            'create launch.json successfully!'
          );
        } catch (e) {
          vscode.window.showErrorMessage(
            `create launch.json failed, ${e.message}`
          );
        }
      }
    ),
